        ...
        for x in 0..width {
            ...
            let is_local_max = if abs_y_scaled < tan_22_x {
                magnitude > magnitude_at(&magnitudes, width, height, x as isize - 1, y as isize)
                    && magnitude
                        >= magnitude_at(&magnitudes, width, height, x as isize + 1, y as isize)
            } else {
                let tan_67_x = tan_22_x + (abs_x << 16);
                if abs_y_scaled > tan_67_x {
                    magnitude > magnitude_at(&magnitudes, width, height, x as isize, y as isize - 1)
                        && magnitude
                            >= magnitude_at(&magnitudes, width, height, x as isize, y as isize + 1)
                } else {
...
fn magnitude_at(magnitudes: &[i32], width: usize, height: usize, x: isize, y: isize) -> i32 {
...
fn sobel_gradients(values: &[u8], width: usize, height: usize) -> (Vec<i32>, Vec<i32>) {
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            gradient_x[index] = -top_left + top_right - 2 * middle_left + 2 * middle_right
                - bottom_left
                + bottom_right;
            gradient_y[index] =
...
fn dilate_binary_edges(edges: &[u8], width: usize, height: usize, kernel_size: usize) -> Vec<u8> {
